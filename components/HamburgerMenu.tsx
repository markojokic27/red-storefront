...
> = ({ headerTheme = 'dark', ...rest }) => (
  <RadixDialog.Root {...rest}>
    <RadixDialog.Trigger className="group h-6 w-6 focus:outline-none">
      ...
      <Icon
        name="x"
        className="hidden group-data-[state=open]:block hover:cursor-pointer"
      />
    </RadixDialog.Trigger>
      <RadixDialog.Overlay>
        <RadixDialog.Content
          className={`hamburger-menu fixed bottom-0 left-0 top-0 z-[70] -translate-x-full transform bg-blue-700 transition-transform duration-300 ease-in-out data-[state=open]:translate-x-0`}
        >
          <RadixDialog.Title />
